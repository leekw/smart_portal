...
var GridAction = {
	_vaildAction : function() {
		var grid = Ext.getCmp('cr-grid');
		var sm = grid.getSelectionModel();
    	var rec = sm.getSelection()[0];
    	if (rec) {
    		if (rec.data.jiraStatus != '요청') {
    			Ext.Msg.alert('Info', 'WBS-CR이 요청 상태일때만 수정이 가능합니다.');
        		return false;
    		}
    	}
    	return true;
	...
	_openSearchGrid : function() {
		var win = Ext.getCmp('search-program-info');
		if (win == null) {
			win = Ext.create('Ext.window.Window', {
				id : 'search-program-info',
	    	    title: '원본 프로그램',
	    	    resizable : true,
	    	    autoScroll: true,
	    	    maximizable : true,
...
Ext.define('Ext.programcr.view.ProgramPanel', {
		   ...
		   tools : [
				{
				    ...
				    tooltip: '원본 프로그램 조회/대상추가 선택',
				    width: 20,
				    height: 20,
				    handler : function() {
				    	if (GridAction._vaildAction()) {
				    		GridAction._openSearchGrid();
				    	}
				    }
				},{
				    xtype: 'tool',                                    
				    ...
				    handler : function() {
				    	if (GridAction._vaildAction()) {
				            ...
				            var r = Ext.create('Ext.programcr.model.ProgramListJira', {
				            	mode : 'I'
				            });
